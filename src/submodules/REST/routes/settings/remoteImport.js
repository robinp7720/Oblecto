...
export default (server, oblecto) => {
    server.get('/remote-import/all/movies', authMiddleWare.requiresAuth, function (req, res) {
        oblecto.seedboxController.importAllMovies();
        res.send([true]);
    });

    // API Endpoint to request a re-index of certain library types
    server.get('/remote-import/:device/movies', authMiddleWare.requiresAuth, function (req, res) {
        oblecto.seedboxController.importMovies(oblecto.seedboxController.seedBoxes[req.params.device]);
        res.send([true]);
    });

    server.get('/remote-import/all/series', authMiddleWare.requiresAuth, function (req, res) {
        oblecto.seedboxController.importAllEpisodes();
        res.send([true]);
    });

    server.get('/remote-import/:device/series', authMiddleWare.requiresAuth, function (req, res) {
